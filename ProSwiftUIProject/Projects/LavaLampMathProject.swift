...
extension Array: VectorArithmetic, AdditiveArithmetic where Element == Double {
    ...
    // We don't need this but have to implement it.. SOLID???
...
struct AnimatablePolygonShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        Path { path in
            ...
            let radius = min(center.y, center.y)
            ...
            let lines = animatableData.enumerated().map { index, value in
                ...
                // xPos / yPos -> regular polygon shapes
                // xPos * value / yPos * value -> irregular polygons
...
struct LavaLampMathProject: View {
...
struct LavaLampMathProject_Previews: PreviewProvider {
